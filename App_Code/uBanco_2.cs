...
/// <summary>
/// Summary description for uBanco_2
/// </summary>
public class uBanco_2
{
    ...
    public static DataSet PegaDadosBancos(Int32 xID)
    {
        ...
        if (xID == 0)
        {
            script = "SELECT * FROM bancos_2 WITH(NOLOCK) order by cod_banco";
        }
        else
        {
            script = "SELECT * FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + Convert.ToString(xID) + " order by cod_banco";
        }
    ...
    {
        return SreDblib.GetDsScript("select apelido from bancos_2");
    }
    ...
    public static ASPxComboBox MontaComboBanco(ASPxComboBox pCombo)
    {
        ...
        return SreDblib.MontaCombo(pCombo, "bancos_2", "nome", "cod_banco", true, " order by cod_banco ");

    }

    public static String PegaNomeID(string xID)
    {
        string script = "SELECT nome FROM bancos_2 WITH(NOLOCK) WHERE ID = " + xID;
        try
        {
    ...
    {
        string script = "SELECT nome FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return SreDblib.GetCampoScript(script);
        }
    ...
    public static String PegaCaminhoRemessa(string xID)
    {
        ...
        string script = "select * from bancos_2 " +
                        " with(nolock) " +
                        " where cod_banco = " + xID;
        string retorno = "";
    ...
    {
        string script = "select sequencia from bancos_2 " +
                        " with(nolock) " +
                        " where cod_banco = " + xID;
        string retorno = "";
        try
    ...
    {
        string script = "SELECT caixa_postal FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return SreDblib.GetCampoScript(script);
        }
    ...
    {
        string script = "SELECT carteira FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return SreDblib.GetCampoScript(script);
        }
    ...
    {
        string script = "SELECT numero_arquivo FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
    ...
    {
        string script = "UPDATE bancos_2 SET (numero_arquivo = numero_arquivo + 1) WHERE cod_banco = " + xID;
    ...
    {
        string script = "SELECT assunto FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return SreDblib.GetCampoScript(script);
        }
    ...
    {
        string script = "SELECT multa FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return SreDblib.GetCampoScript(script);
        }
    ...
    {
        string script = "SELECT comissao FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return SreDblib.GetCampoScript(script);
        }
    ...
    {
        string script = "SELECT juros FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return SreDblib.GetCampoScript(script);
        }
    ...
    {
        string script = "SELECT sequencia FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return SreDblib.GetCampoScript(script);
        }
    ...
    {
        string script = "SELECT prazo_fol FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
    ...
    {
        string script = "SELECT prazo_pro FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return Srelib.QStr0(SreDblib.GetCampoScript(script).Trim(), 2);
        }
    ...
    {
        string script = "SELECT prazo_dev FROM bancos_2 WITH(NOLOCK) WHERE cod_banco = " + xID;
        try
        {
            return Srelib.QStr0(SreDblib.GetCampoScript(script).Trim(), 2);
        }
