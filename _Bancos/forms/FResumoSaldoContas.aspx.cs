...
public partial class FResumoSaldoContas : System.Web.UI.Page
{
    DataSet dscreceber;
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        lAltera = false; // SreDblib.TemPermissao("001.008.002.000", (String)Session["CodUsuario"]);
    ...
    private void AtualizaGrid2()
    {
        ...
        //Todas as Contas
        //
        dscreceber = uContas.PegaUltimoSaldoContas();
        //

        for (int i = 0; i < dscreceber.Tables[0].Rows.Count - 1; i++)
        {
            saldo = saldo + Convert.ToDecimal(dscreceber.Tables[0].Rows[i]["saldo_f"]);
            aplicdesbloqueada = aplicdesbloqueada + Convert.ToDecimal(dscreceber.Tables[0].Rows[i]["saldo_aplicacaoD"]);
            aplicbloqueada = aplicbloqueada + Convert.ToDecimal(dscreceber.Tables[0].Rows[i]["saldo_aplicacaoB"]);
            saldofinaltotal = saldofinaltotal +
                              Convert.ToDecimal(dscreceber.Tables[0].Rows[i]["saldo_f"]) +
                              Convert.ToDecimal(dscreceber.Tables[0].Rows[i]["saldo_aplicacaoD"]) +
                              Convert.ToDecimal(dscreceber.Tables[0].Rows[i]["saldo_aplicacaoB"]);
        }
        ...
        GridListagem.KeyFieldName = "cod_conta";
    ...
    private void AtualizaGrid()
    {
        ...
        //Conta Específica
        //
        dscreceber = uContas.PegaDadosConta(cbcontascorrentes2,
        ...
        saldo = Convert.ToDecimal(dscreceber.Tables[0].Rows[dscreceber.Tables[0].Rows.Count - 1]["saldo_f"]);
        saldolimite = saldo + limite;
        aplicdesbloqueada = Convert.ToDecimal(dscreceber.Tables[0].Rows[dscreceber.Tables[0].Rows.Count - 1]["saldo_aplicacaoD"]);
        aplicbloqueada = Convert.ToDecimal(dscreceber.Tables[0].Rows[dscreceber.Tables[0].Rows.Count - 1]["saldo_aplicacaoB"]);
        saldofinaltotal = saldolimite + aplicbloqueada + aplicdesbloqueada;

        Label14.Text = "Saldo Em " + dscreceber.Tables[0].Rows[dscreceber.Tables[0].Rows.Count - 1]["data"].ToString().Substring(0, 10);
        edsaldot.Text = saldo.ToString();
        //
