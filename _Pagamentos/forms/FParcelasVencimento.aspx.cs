...
public partial class FParcelasVencimento : System.Web.UI.Page
{
    DataSet dsparcelas;
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        {
            //Dia do Mês
            eddiavencimento.Text = String.Format("{0:00}", Convert.ToInt16(eddiavencimento.Text));
            Session.Remove("dia");
            if ((eddiavencimento.Text != string.Empty) ||
                (eddiavencimento.Text != "0"))
            {
                if ((Convert.ToInt16(eddiavencimento.Text) >= 1) &&
                    (Convert.ToInt16(eddiavencimento.Text) <= 31))
                {
                    Session.Add("dia", eddiavencimento.Text.Trim());
                }
                else
                {
                    lOk = false;
                    //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Dia aceito entre 1 e 31, Dia Inválido!');", true);
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                    "toastr.info('Dia aceito entre 1 e 31, Dia Inválido!', 'Atenção!')", true);
                }
            }
            else
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Dia da Parcela em Branco!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Dia da Parcela em Branco!', 'Atenção!')", true);
        ...
        if (lOk)
        {
            ...
            if (parte == "0")
            {
                uParcelas.GravaNovoDiaVencimento(cbproduto.Value.ToString(),
            ...
            if (parte == "0")
            {
                fwObs_Log = "VENCIMENTO PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte +
                            " DIA : " + eddiavencimento.Text;
            }
            else
            {
                fwObs_Log = "VENCIMENTO PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote +
                            " DIA : " + eddiavencimento.Text;
            }
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ), 
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "PARCELAS"                            ),
                                         new SqlParameter("@obs"      , "VENCIMENTO PARCELA"                  ),
            ...
            //
            btprocessa.Enabled = false;
            //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Novo Vencimento Registrado!');", true);
            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
            "toastr.success('Novo Vencimento Registrado!', 'Atenção!')", true);
