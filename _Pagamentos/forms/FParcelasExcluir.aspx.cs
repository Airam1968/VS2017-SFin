...
public partial class FParcelasExcluir : System.Web.UI.Page
{
    DataSet dsparcelas;
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            //
            //Fim Parcela selecionada
        }
        if (lOk)
        {
            //
            DSparcela = uParcelas.PegaDadosParcelaID(cbproduto.Value.ToString(), cbgrupo.Value.ToString(), cbgrupounidades.Value.ToString(), "0", tst);
            //Carregar Dados
            string parcela = DSparcela.Tables[0].Rows[0]["parcela"].ToString();
            string tipo = DSparcela.Tables[0].Rows[0]["tipo"].ToString();
            string dtvencimento = DSparcela.Tables[0].Rows[0]["data_vencimento"].ToString().Substring(0, 10);
            string dtpagamento = DSparcela.Tables[0].Rows[0]["data_pagamento"].ToString().Substring(0, 10);
            decimal vlr_pagamento = Convert.ToDecimal(DSparcela.Tables[0].Rows[0]["vlr_pagamento"]);
            //
            //Parcela Ativa
            //Carregar Dados
            Boolean ativa = Convert.ToBoolean(DSparcela.Tables[0].Rows[0]["ativa"]);

            if (!ativa)
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Não é possivel Cancelar Parcela Inativa!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Não é possivel Cancelar Parcela Inativa!', 'Atenção!')", true);
                //Limpar Campos
            }
            //
            if (lOk)
            {
                //Processa Exclusão da Parcela
                if ((dtpagamento == "01/01/1900") && (vlr_pagamento == 0))
                {
                    if (parte == "0")
                    {
                        uParcelas.Exclui(cbproduto.Value.ToString(),
                    ...
                    //Fim Processa Exclusão da Parcela
                    //
                    // CRIA LOG ===============================================================================================================================================
                    if (parte == "0")
                    {
                        fwObs_Log = "EXCLUIR PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte +
                                    " PARCELA ID: " + tst + " PARCELA: " + parcela + "/" + tipo;
                    }
                    else
                    {
                        fwObs_Log = "EXCLUIR PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote +
                                    " PARCELA ID: " + tst + " PARCELA: " + parcela + "/" + tipo;
                    }
                    SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ),
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "PARCELAS"                            ),
                                         new SqlParameter("@obs"      , "EXCLUIR PARCELA"                     ),
                    ...
                    //btprocessa.Enabled = false;
                    //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Exclusão de Parcela Registrada!');", true);
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                    "toastr.success('Exclusão de Parcela Registrada!', 'Atenção!')", true);
                }
                else
                {
                    //btprocessa.Enabled = false;
                    //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Parcela Paga, Não é possível Excluir!');", true);
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                    "toastr.error('Parcela Paga, Não é possível Excluir!', 'Atenção!')", true);
