...
public partial class FParcelasContabil : System.Web.UI.Page
{
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            else
            {
                ...
                else
                {
                    //Desmarcado
                    lOk = false;
                    //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Ação de Periodicidade Não Selecionada!');", true);
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                    "toastr.info('Ação de Periodicidade Não Selecionada!', 'Atenção!')", true);
                }
        ...
        if (lOk)
        {
            ...
            if (parte == "0")
            {
                uParcelas.GravaPeriodicidade(cbproduto.Value.ToString(),
                                             ...
                                             rbperiodicidade.SelectedIndex.ToString());
            }
            else
            {
                uParcelas.GravaPeriodicidade(cbproduto.Value.ToString(),
            ...
            if (parte == "0")
            {
                fwObs_Log = "PERIODICIDADE PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte;
            ...
            {
                fwObs_Log = "PERIODICIDADE PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote;
                fwObs_Log = fwObs_Log + " " + Convert.ToString(dtprevisao1.Value).Substring(0, 10) + " até " + Convert.ToString(dtprevisao2.Value).Substring(0, 10);

            }
            if (rbperiodicidade.SelectedIndex.ToString() == "0")
            {
                fwObs_Log = fwObs_Log + " " + "a.d.";
            }
            if (rbperiodicidade.SelectedIndex.ToString() == "1")
            {
                fwObs_Log = fwObs_Log + " " + "a.m.";
            }
            if (rbperiodicidade.SelectedIndex.ToString() == "2")
            {
                fwObs_Log = fwObs_Log + " " + "a.a.";
            }
            //
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ),
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "PARCELAS"                            ),
                                         new SqlParameter("@obs"      , "PERIODICIDADE PARCELA"               ),
            ...
            //
            btprocessa.Enabled = false;
            //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Periodicidade Registrada!');", true);
            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
            "toastr.success('Periodicidade Registrada!', 'Atenção!')", true);
        }
