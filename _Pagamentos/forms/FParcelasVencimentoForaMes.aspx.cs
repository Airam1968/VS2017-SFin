...
public partial class FParcelasVencimentoForaMes : System.Web.UI.Page
{
    DataSet dsparcelas;
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            if (dtprevisao1.Text != string.Empty)
            {
                Session.Add("dia", eddiavencimento.Value.ToString().Trim());
            }
            else
            {
                Session.Add("dia", "01/01/1900");
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Data Inicial em Branco!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Data Inicial em Branco!', 'Atenção!')", true);
        ...
        if (lOk)
        {
            ...
            if (parte == "0")
            {
                uParcelas.GravaNovoDiaVencimentoForaMes(cbproduto.Value.ToString(),
                                                        ...
                                                        Convert.ToString(eddiavencimento.Value).Substring(0, 10));
            }
            else
            {
                uParcelas.GravaNovoDiaVencimentoForaMes(cbproduto.Value.ToString(),
            ...
            if (parte == "0")
            {
                fwObs_Log = "VENCIMENTO PARCELA FORA O MÊS: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte +
                            " DIA : " + eddiavencimento.Text;
            }
            else
            {
                fwObs_Log = "VENCIMENTO PARCELA FORA O MÊS: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote +
                            " DIA : " + eddiavencimento.Text;
            }
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ), 
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "PARCELAS"                            ),
                                         new SqlParameter("@obs"      , "VENCIMENTO PARCELA FORA O MÊS"       ),
