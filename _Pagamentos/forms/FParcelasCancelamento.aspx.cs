...
public partial class FParcelasCancelamento : System.Web.UI.Page
{
    DataSet dsparcelas;
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            //
            DSparcela = uParcelas.PegaDadosParcelaID(cbproduto.Value.ToString(), cbgrupo.Value.ToString(), cbgrupounidades.Value.ToString(), parte, tst);
            //Carregar Dados
            string parcela = DSparcela.Tables[0].Rows[0]["parcela"].ToString();
            string tipo = DSparcela.Tables[0].Rows[0]["tipo"].ToString();
            string vlr_parcela = DSparcela.Tables[0].Rows[0]["vlr_parcela"].ToString();
        ...
        if (lOk)
        {
            ...
            //Fim Processa Cancelamento
            //
            // CRIA LOG ===============================================================================================================================================
            if (parte == "0")
            {
                fwObs_Log = "CANCELAMENTO PAGAMENTO PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte +
                            " PARCELA ID: " + tst;
            }
            else
            {
                fwObs_Log = "CANCELAMENTO PAGAMENTO PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote +
                          " PARCELA ID: " + tst;
            }
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ),
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "PARCELAS"                            ),
                                         new SqlParameter("@obs"      , "CANCELAMENTO PAGAMENTO PARCELA"      ),
            ...
            //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Cancelamento de Pagamento Registrado!');", true);
            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
            "toastr.success('Cancelamento de Pagamento Registrado!', 'Atenção!')", true);
        }
    }
