...
public partial class FParcelasFluxo : System.Web.UI.Page
{
    DataSet dsparcelas;
    DataSet parcela;
    ...
    protected void btcarrega_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            edparcelatipo.Enabled = false;
            edvlr_parcela.Enabled = true;
            eddtvencimento.Enabled = true;
            edfeedback.Enabled = true;

            edtxmulta.Enabled = false;
            edtxcomissao.Enabled = false;
            //
            ...
            //Carregar Dados
            edparcela.Text = parcela.Tables[0].Rows[0]["parcela"].ToString();
            edparcelatipo.Text = parcela.Tables[0].Rows[0]["tipo"].ToString();
            ...
            //
            edfeedback.Text = "Parcela Selecionada, preencha dos campos e clique no Botão Processar.";
            //
            edvlr_parcela.Focus();
            //
        }
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            //Processa Alteração de Parcela
            //Parcela 1
            if (parte == "0")
            {
                uParcelas.AlteraParcela(IDParcela,
            ...
            if (parte == "0")
            {
                fwObs_Log = "ALTERAÇÃO PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte +
                            " PARCELA No./TIPO: " + edparcela.Text + "/" + edparcelatipo.Text;
            }
            else
            {
                fwObs_Log = "ALTERAÇÃO PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote +
                            " PARCELA No./TIPO: " + edparcela.Text + "/" + edparcelatipo.Text;
            }
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ), 
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "PARCELAS"                            ),
                                         new SqlParameter("@obs"      , "ALTERAÇÃO PARCELA"                   ),
            ...
            btprocessa.Enabled = false;
            //
            edfeedback.Text = "Alteração de Parcela Registrada!";
            //
            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
            "toastr.success('Alteração de Parcela Registrada!', 'Atenção!')", true);
        }
