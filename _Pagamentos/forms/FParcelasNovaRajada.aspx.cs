...
public partial class FParcelasNovaRajada : System.Web.UI.Page
{
    DataSet dsparcelas;
    DataSet parcela;
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            if ((edqtdparcelas.Text == string.Empty) || (edparcela.Text == "0"))
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Quantidade de Parcelas Vazio ou Zerado!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Quantidade de Parcelas Vazio ou Zerado!', 'Atenção!')", true);
            }
            else
            {
                try
                {
                    qtd_parcela = Convert.ToInt64(edqtdparcelas.Text);
                }
                catch (Exception)
                {
                    lOk = false;
                    //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Quantidade de Parcelas Não é um inteiro Válido!');", true);
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                    "toastr.info('Quantidade de Parcelas Não é um inteiro Válido!', 'Atenção!')", true);
                }
        ...
        {
            //Numero da Parcela
            if ((edparcela.Text == string.Empty) || (edparcela.Text == "0"))
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Número de Parcela Vazio ou Zerado!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Número de Parcela Vazio ou Zerado!', 'Atenção!')", true);
            }
            else
            {
                try
                {
                    num_parcela = Convert.ToInt64(edparcela.Text);
                }
                catch (Exception)
                {
                    lOk = false;
                    //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Número de Parcela Não é um inteiro Válido!');", true);
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                    "toastr.info('Número de Parcela Não é um inteiro Válido!', 'Atenção!')", true);
                }
        ...
        if (lOk)
        {
            edparcelatipo.Text = edparcelatipo.Text.ToUpper();
            //Tipo da Parcela
            if (edparcelatipo.Text == string.Empty)
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Tipo de Parcela Vazio!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Tipo de Parcela Vazio!', 'Atenção!')", true);
            }
            else
            {
                if ((edparcelatipo.Text.IndexOf("N") == -1) &&
                   (edparcelatipo.Text.IndexOf("I") == -1) &&
                   (edparcelatipo.Text.IndexOf("S") == -1) &&
                   (edparcelatipo.Text.IndexOf("R") == -1))
                {

                    lOk = false;
                    //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Tipo de Parcela Não Válido, Tipos Aceitavéis (S,N,I,R)!');", true);
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                    "toastr.info('Tipo de Parcela Não Válido, Tipos Aceitavéis (S,N,I,R)!', 'Atenção!')", true);
                }
        ...
        {
            //Valor da Parcela
            if ((edvlr_parcela.Text == string.Empty) || (edvlr_parcela.Text == "0,00"))
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Valor da Parcela Vazio ou Zerado!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Valor da Parcela Vazio ou Zerado!', 'Atenção!')", true);
            }
        }
        //
        ...
        {
            //Taxa Multa
            if (edtxmulta.Text == string.Empty)
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Taxa de Multa Vazia!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Taxa de Multa Vazia!', 'Atenção!')", true);
            }
        }
        //
        if (lOk)
        {
            //Taxa Comissão
            if (edtxcomissao.Text == string.Empty)
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Taxa de Comissão de Permanência Vazia!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Taxa de Comissão de Permanência Vazia!', 'Atenção!')", true);
            }
        }
        //
        ...
        if (lOk)
        {
            ...
            {
                if (rbperiodicidadecomissao.SelectedIndex == 2)
                {
        ...
        if (lOk)
        {
            vencimento = Convert.ToDateTime(eddtvencimento.Text.Substring(0, 10));
            //Processa Nova Parcela
            for (int i = 0; i < qtd_parcela; i++)
            {
                //Parcela N
                if (parte == "0")
                {
                    uParcelas.Grava(cbproduto.Value.ToString(),
                                    ...
                                    parte,
                                    num_parcela.ToString().Trim(),
                                    edparcelatipo.Text,
                                    vencimento.ToString().Substring(0,10),
                                    Math.Round(Convert.ToDecimal(edvlr_parcela.Text), 2),
                                    "01/01/1900",
                                    0,
                                    "01/01/1900",
                                    Math.Round(Convert.ToDecimal(edvlr_parcela.Text), 2),
                                    ...
                                    0,
                                    1,
                                    Convert.ToDecimal(edtxmulta.Text),
                                    Convert.ToDecimal(edtxcomissao.Text),
                                    0,
                                    0,
                                    0,
                ...
                if (parte == "0")
                {
                    fwObs_Log = "NOVA PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte +
                                " PARCELA No./TIPO: " + num_parcela.ToString() + "/" + edparcelatipo.Text;
                }
                else
                {
                    fwObs_Log = "NOVA PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote +
                                " PARCELA No./TIPO: " + num_parcela.ToString() + "/" + edparcelatipo.Text;
                }
                SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ), 
                                             new SqlParameter("@tabela"   , fWtabela                              ),
                                             new SqlParameter("@operacao" , "PARCELAS"                            ),
                                             new SqlParameter("@obs"      , "NOVA PARCELA"                        ),
            ...
            //
            btprocessa.Enabled = false;
            //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Parcelas Novas Registradas!');", true);
            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
            "toastr.success('Parcelas Novas Registradas!', 'Atenção!')", true);
        }
    ...
    protected void rbparcelasituacao_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        edtxmulta.Text = "0,00";
        edtxcomissao.Text = "0,00";
        edqtdparcelas.Text = "0";
        edfeedback.Text = "";
        edtxmultaperiodicidade.Text = "a.m.";
        //
        edparcela.Enabled = true;
        edparcelatipo.Enabled = true;
        edvlr_parcela.Enabled = true;
        eddtvencimento.Enabled = true;
        eddtbase.Enabled = false;
        edindexador.Enabled = false;
        edperiodicidade.Enabled = false;
        edtxmulta.Enabled = true;
        edtxcomissao.Enabled = true;
        edqtdparcelas.Enabled = true;
        edfeedback.Enabled = true;
        edtxmultaperiodicidade.Enabled = false;
        rbperiodicidadecomissao.Enabled = false;
        //
        Label23.Visible = true;
        edparcela.Visible = true;
        edparcelatipo.Visible = true;
        Label5.Visible = true;
        ...
        edtxmulta.Visible = true;
        Label14.Visible = true;
        edtxcomissao.Visible = true;
        Label15.Visible = true;
        edqtdparcelas.Visible = true;
        Label24.Visible = true;
        edfeedback.Visible = true;
        edtxmultaperiodicidade.Visible = true;
        rbperiodicidadecomissao.Visible = true;
        //
        //Carregar Dados
        eddtvencimento.Value = DateTime.Now;
        ...
        //
        edtxmulta.Text = Convert.ToString(uParametros.PegaParametro("MULTA"));
        edtxcomissao.Text = Convert.ToString(uParametros.PegaParametro("COMISSAO TAXA"));
        edtxmultaperiodicidade.Text = "a.m.";
        btprocessa.Enabled = true;
        //
        edfeedback.Text = "Preencha dos campos e clique no Botão Processa.";
        //
        edparcela.Focus();
        //
