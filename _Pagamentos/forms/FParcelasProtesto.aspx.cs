...
public partial class FParcelasProtesto : System.Web.UI.Page
{
    DataSet dsparcelas;
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            else
            {
                ...
                else
                {
                    //Desmarcado
                    lOk = false;
                    //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Ação de Protesto Não Selecionada!');", true);
                    ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                    "toastr.info('Ação de Protesto Não Selecionada!', 'Atenção!')", true);
                }
            }
        }
        //
        if (lOk)
        {
            ...
            if (parte == "0")
            {
                uParcelas.GravaProtesto(cbproduto.Value.ToString(),
            ...
            if (parte == "0")
            {
                fwObs_Log = "PROTESTO PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte;
            ...
            {
                fwObs_Log = "PROTESTO PARCELA: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote;
                fwObs_Log = fwObs_Log + " " + Convert.ToString(dtprevisao1.Value).Substring(0, 10) + " até " + Convert.ToString(dtprevisao2.Value).Substring(0, 10);

            }
            if (rbprotesto.SelectedIndex.ToString() == "0")
            {
                fwObs_Log = fwObs_Log + " " + "Protestar";
            }
            else
            {
                fwObs_Log = fwObs_Log + " " + "Não Protestar";
            }
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ), 
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "PARCELAS"                            ),
                                         new SqlParameter("@obs"      , "PROTESTO PARCELA"                    ),
            ...
            //
            btprocessa.Enabled = false;
            //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Protesto Registrado!');", true);
            ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
            "toastr.info('Protesto Registrado!', 'Atenção!')", true);
