...
public partial class FParcelas : System.Web.UI.Page
{
    DataSet dsparcelas;
        ...
        if (cbproduto.Text != string.Empty)
        {
            if (cbgrupo.Text != string.Empty)
            {
                if (cbgrupounidades.Text != string.Empty)
                {
                    ...
                    edcorrecao.Text = "";
                    edcorrecao.Text = edcorrecao.Text + uVendas.PegaPeriodicidade(cbproduto.Value.ToString(), cbgrupo.Value.ToString(), cbgrupounidades.Value.ToString(), "0", "1");
                    edcorrecao.Text = edcorrecao.Text + " - " + uVendas.PegaIndexador(cbproduto.Value.ToString(), cbgrupo.Value.ToString(), cbgrupounidades.Value.ToString(), "0");
                    edcorrecao.Text = edcorrecao.Text + " - BASE: ";
                    edcorrecao.Text = edcorrecao.Text + uVendas.PegaDataBase(cbproduto.Value.ToString(), cbgrupo.Value.ToString(), cbgrupounidades.Value.ToString(), "0").ToString().Substring(0,10);
                    //
    ...
    private void AtualizaGrid(String situacaopgto)
    {
        ...
        dsparcelas = uParcelas.PegaDadosParcelas2(cbproduto.Value.ToString().Trim(),
                                                 ...
                                                 false,
                                                 "0");
        //Limpar os Dados da exibição anterior
        GridListagem.DataSourceID = null;
    ...
    protected void btcalcula_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            if (!ativa)
            {
                ...
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.error('Não é possivel Pagar Parcela Inativa!', 'Atenção!')", true);
                //Limpar Campos
            }
        }
        //
    ...
    protected void btimprime_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            else
            {
                ...
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Parcela Não Selecionada!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.error('Parcela Não Selecionada!', 'Atenção!')", true);
            }
        }
        //
        if (lOk)
