...
public partial class FAReceberLancamentosExcluir : System.Web.UI.Page
{
    DataSet dscreceber;
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            //Processa Exclusão do Lançamento
            uContasReceber.ExcluiLancamento(edlancamento.Text);
            //
            // CRIA LOG ===============================================================================================================================================
            fwObs_Log = "LANÇAMENTO CONTAS A RECEBER - EXCLUIR: " + edlancamento.Text;
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ), 
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "CONTAS A RECEBER"                    ),
                                         new SqlParameter("@obs"      , "EXCLUIR LANÇAMENTO CONTAS A RECEBER" ),
    ...
    protected void edlancamento_TextChanged(object sender, EventArgs e)
    {
        ...
        if (edlancamento.Text.Trim() != String.Empty)
        {
            ...
            if (dscreceber.Tables[0].Rows.Count > 0)
            {
                ...
                //
                DateTime temp = Convert.ToDateTime(dscreceber.Tables[0].Rows[0]["data_prevista"]);
                try
                {
            ...
            else
            {
                ...
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                                    "toastr.info('Lançamento Não Existe!', 'Atenção!')", true);
