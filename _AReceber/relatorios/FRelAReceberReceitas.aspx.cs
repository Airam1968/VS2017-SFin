...
public partial class FRelAReceberReceitas : System.Web.UI.Page
{
    DataSet dsreceitas;                                    // DATASETS DOS LOOKUPS TEM QUE SER CRIADOS AQUI  ====================================================================================
    String login_usuario;
    Boolean lImprime = false;
    String fwObs_Log = "";
    String fWtabela = "CLASSE_RECEITAS";
    ...
    protected void Page_Init(object sender, EventArgs e)  // CARGA DOS LOOKUPS TEM QUE SER FEITA NESTE EVENTO  ====================================================================================
    {
        ...
        // CARGA DE UM LOOKUP  ===============================================================================================================================================
        // lookupempresa
        dsreceitas = uReceitas.LookUpReceitas();    // Método que retorna o dataset com os campos pro lookup
        lookupempresa.DataSource = dsreceitas;
    ...
    protected void btImpressao_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            //Receitas
            tst = "";
            Session.Remove("cbreceitas");
            List<object> cbreceitas = lookupempresa.GetSelectedFieldValues(new string[] { "codigo" });
            if (cbreceitas.Count != dsreceitas.Tables[0].Rows.Count)  // VERIFICA SE TODOS OS ELEMENTOS FORAM SELECIONADOS  ===============================================================================================================================================
            {
                foreach (Int16 xval in cbreceitas)
                {
                    if (tst != "")
                    {
                        tst = tst + ",";
            ...
            {
                if ((cbreceitas.Count == 1) && (dsreceitas.Tables[0].Rows.Count == 1))
                {
                    foreach (Int16 xval in cbreceitas)
                    {
                        if (tst != "")
