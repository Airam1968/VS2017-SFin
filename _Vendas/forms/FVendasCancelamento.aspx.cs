...
public partial class FVendasCancelamento : System.Web.UI.Page
{
    DataSet dsunidade;
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            {
                lOk = false;
                //ScriptManager.RegisterStartupScript(UpdatePanel, GetType(), "msgbox", "alert('Financiamento possui parcelas pagas, não pode ser excluído!');", true);
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                "toastr.info('Financiamento possui parcelas pagas, não pode ser excluído!', 'Atenção!')", true);
            }
        }

        //Processa Cancelamento da Venda
        if (lOk)
        {
            if (parte == "0")
            {
                uVendas.CancelaVenda(cbproduto.Value.ToString(), cbgrupo.Value.ToString(), cbgrupounidades.Value.ToString(), parte);
            }
            else
            {
                uVendas.CancelaVenda(cbproduto.Value.ToString(), cbgrupo.Value.ToString(), cbgrupounidades.Value.ToString(), meiolote);
            }
            //
            // CRIA LOG ===================================================================================================================================================
            if (parte == "0")
            {
                fwObs_Log = "CANCELAMENTO VENDA UNIDADE: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + parte;
            }
            else
            {
                fwObs_Log = "CANCELAMENTO VENDA UNIDADE: " + cbproduto.Value.ToString() + "/" + cbgrupo.Value.ToString() + "/" + cbgrupounidades.Value.ToString() + "/" + meiolote;
            }
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ),
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "VENDAS"                              ),
                                         new SqlParameter("@obs"      , "CANCELAMENTO VENDA UNIDADE"          ),
                                         new SqlParameter("@acao"     , fwObs_Log                             ) };
            uLog.MakeLog(param_log);
            // FIM CRIA LOG ===============================================================================================================================================

        }
        //Fim Processa Cancelamento da Venda
        if (lOk)
        {
            btprocessa.Enabled = false;
