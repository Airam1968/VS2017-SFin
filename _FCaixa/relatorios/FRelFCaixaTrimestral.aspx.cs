...
public partial class FRelFCaixaTrimestral : System.Web.UI.Page
{
    ...
    protected void btImpressao_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            //
            //Trimestre
            Session.Remove("rbtrimestre");
            Session.Add("rbtrimestre", rbtrimestre.SelectedIndex + 1);
        }
        //
        if (lOk)
        {
        ...
        if (lOk)
        {
            ...
            fwObs_Log = "RELATÓRIO FLUXO DE CAIXA - TRIMESTRE: " + cbano.Text.Trim() + " - " + rbtrimestre.SelectedIndex.ToString();
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]           ), 
                                         new SqlParameter("@tabela"   , fWtabela                                ),
                                         new SqlParameter("@operacao" , "RELATÓRIO"                             ),
                                         new SqlParameter("@obs"      , "RELATÓRIO FLUXO DE CAIXA - TRIMESTRE"  ),
                                         new SqlParameter("@acao"     , fwObs_Log                               ) };
            uLog.MakeLog(param_log);

            // FIM CRIA LOG RELATÓRIO  ===============================================================================================================================================

            ScriptManager.RegisterStartupScript(this, typeof(string), "OPEN_WINDOW", "window.open( '" + HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority) + HttpRuntime.AppDomainAppVirtualPath + "/_FCaixa/relatorios/RelFCaixaTrimestral.aspx', null, 'height=755,width=900,status=yes,resizable=yes,menubar=yes,location=yes' );", true);
