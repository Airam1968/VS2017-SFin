...
public partial class FRelApagarMalaDireta : System.Web.UI.Page
{
    ...
    protected void btImpressao_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            List<object> cbfornecedores = lookupempresa.GetSelectedFieldValues(new string[] { "apelido" });
            if (cbfornecedores.Count != dsfornecedores.Tables[0].Rows.Count)  // VERIFICA SE TODOS OS ELEMENTOS FORAM SELECIONADOS  ===============================================================================================================================================
            {
                foreach (string xval in cbfornecedores)
                {
                    if (tst != "")
                    {
                        tst = tst + ",";
