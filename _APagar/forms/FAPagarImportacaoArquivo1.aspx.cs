...
public partial class FAPagarImportacaoArquivo1 : System.Web.UI.Page
{
    ...
    String fwObs_Log = "";
    String fWtabela = "CONTAS A PAGAR";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            try
            {
                loadFolder(lookuparquivos, Server.MapPath(@"~\Arquivos\Importacao\Cpagar\Arquivo_texto\"));
            }
            catch (Exception)
            {
    ...
    protected void btprocessa_Click(object sender, EventArgs e)
    {
        ...
        if (lOk)
        {
            ...
            else
            {
                ...
                ScriptManager.RegisterStartupScript(this.Page, typeof(Page), Guid.NewGuid().ToString(),
                                    "toastr.info('Arquivo Não Selecionado!', 'Atenção!')", true);
            }
        }
        //
        if (lOk)
        {
            ...
            for (int i = 0; i < mensagemLinha.Count; i++)
            {
                ...
                //
                if (mensagemLinha[i].Substring(0, 1) != " ")
                {
                    //
                    ...
                    dr["fornecedor"] = mensagemLinha[i].Substring(13, 6);
                    dr["fornecedor"] = Convert.ToInt64(dr["fornecedor"]).ToString();
                    ...
                    if (dr["data_prevista"].ToString().Trim() != "")
                    {
                        dr["data_prevista"] = dr["data_prevista"].ToString().Trim().Substring(0, 2) + "/" +
                                              dr["data_prevista"].ToString().Trim().Substring(2, 2) + "/" +
                    ...
                    dr["data_documento"] = mensagemLinha[i].Substring(107, 8);
                    if ((dr["data_documento"].ToString().Trim() != "") &&
                        (dr["data_documento"].ToString().Trim() != "00000000"))
                    {
                        dr["data_documento"] = dr["data_documento"].ToString().Trim().Substring(0, 2) + "/" +
                                               dr["data_documento"].ToString().Trim().Substring(2, 2) + "/" +
                    ...
                    dr["data_realizado"] = mensagemLinha[i].Substring(116, 8);
                    if ((dr["data_realizado"].ToString().Trim() != "") &&
                        (dr["data_realizado"].ToString().Trim() != "00000000"))
                    {
                        dr["data_realizado"] = dr["data_realizado"].ToString().Trim().Substring(0, 2) + "/" +
                                               dr["data_realizado"].ToString().Trim().Substring(2, 2) + "/" +
                    ...
                    dr["vlr_realizado"] = mensagemLinha[i].Substring(125, 12);
        ...
        if (lOk)
        {
            ...
            //
            // CRIA LOG ===============================================================================================================================================
            fwObs_Log = "IMPORTAÇÃO ARQUIVO CONTAS A PAGAR - TEXTO TIPO 1 : " + tst;
            SqlParameter[] param_log = { new SqlParameter("@login"    , (String)Session["CodUsuario"]         ), 
                                         new SqlParameter("@tabela"   , fWtabela                              ),
                                         new SqlParameter("@operacao" , "CONTAS A PAGAR"                      ),
                                         new SqlParameter("@obs"      , "IMPORTAÇÃO ARQUIVO CONTAS A PAGAR"   ),
